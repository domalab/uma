...
function omniraid_log($m) {
	global $plugin;
  shell_exec("/usr/bin/logger"." ".escapeshellarg($m)." -t omniraid");
}

$socket_path = "/var/run/omniraid-api.sock";
$action = $_POST['action'] ?? '';
...
if (!$socket) {
  ...
  omniraid_log("service unavailable ${errstr}");
